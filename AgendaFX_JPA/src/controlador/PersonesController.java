...
public class PersonesController{
	...
	public void setConexionBD(EntityManager em) {	
		...
		personesDAO = new PersonesDAO(em);
		...
		personesData = FXCollections.observableList(personesDAO.getList());
	...
	@FXML private void initialize() {
		...
		//Carregar la persona a mida que indiquen el codi, no caldrá fer-ho al pulsar tabulador o enter i arreglem l'error que l'usari sorti del conntrol fent click a fora
		idTextField.textProperty().addListener((observable, oldValue, newValue) -> {
			System.out.println("textfield changed from " + oldValue + " to " + newValue);
			if (!newValue.equals("")) {
				//Comprovar si existeix el producte indicat en el control idTextField
				persona = personesDAO.find(Integer.parseInt(newValue));
		...
		// Quan l'usuari canvia de linia executem el métode mostrarProducte
		...
		//produeix error si no posem a les VM arguments això: --add-opens=javafx.graphics/javafx.scene=org.controlsfx.controls
